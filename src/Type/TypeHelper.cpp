...
#define fromFloatSpecialization(T)                                                                                     \
    template<>                                                                                                         \
    T fromFloatToType(float a) {                                                                                       \
        return _concat(floatTo, T)(a);                                                                                 \
    }
...
template<>
uint32_t fromString<uint32_t>(const std::string &s) {
    static bool l = set_locale();
    char *pos = nullptr;
...
void fromStringImpl(std::string s, FixedType &f) {
    int N = sizeof(f.words) / sizeof(uint32_t);
    for (int i = 0; i < N; i++) { f.words[i] = 0; }
...
template<>
Fixed4 fromString<Fixed4>(const std::string &s) {
    Fixed4 result;
...
template<>
Fixed8 fromString<Fixed8>(const std::string &s) {
    Fixed8 result;
    fromStringImpl(s, result);
    return result;
}
...
template<>
Fixed16 fromString<Fixed16>(const std::string &s) {
    Fixed16 result;
    fromStringImpl(s, result);
    return result;
}
...
template<>
std::string toString<float>(const float &s) {
...
template<>
std::string toString<int>(const int &s) {
    return std::to_string(s);
}

template<>
std::string toString<uint32_t>(const uint32_t &s) {
    return std::to_string(s);
}

template<>
std::string toString<double>(const double &s) {
    static std::locale l = std::locale::global(std::locale());
    std::stringstream ss;
...
template<>
std::string toString<std::string>(const std::string &s) {
    return s;
}

template<typename FixedType>
std::string toStringImpl(FixedType f) {
...
template<>
std::string toString<Fixed4>(const Fixed4 &s) {
    return toStringImpl(s);
}

template<>
std::string toString<Fixed8>(const Fixed8 &s) {
    return toStringImpl(s);
}

template<>
std::string toString<Fixed16>(const Fixed16 &s) {
    return toStringImpl(s);
}
...
template<>
std::string toString<bool>(const bool &b) {
    return b ? "true" : "false";
}
...
template<>
longdouble fromString<longdouble>(const std::string &s) {
    char *rest = nullptr;
    longdouble f = std::strtold(s.c_str(), &rest);
    if (rest != s.c_str() + s.size()) { throw std::runtime_error("invalid conversion"); }
    return f;
}
...
template<>
std::string toString<longdouble>(const longdouble &f) {
    char buf[128];
...
template<>
float128 fromString<float128>(const std::string &s) {
    char *rest = nullptr;
...
template<>
std::string toString<float128>(const float128 &f) {
    char buf[128];
...
template<>
qf128 fromString<qf128>(const std::string &s) {
    return qfAssign(fromString<float>(s));
}

template<>
std::string toString<qf128>(const qf128 &f) {
...
template<>
std::shared_ptr<Factory> fromString<std::shared_ptr<Factory>>(const std::string &s) {
    for (auto &factory : Factory::getAllFactories()) {
        if (factory->getDeviceName() == s) { return factory; }
    }
    throw std::runtime_error("factory with name '" + s + "' not found.");
}

template<>
std::string toString<std::shared_ptr<Factory>>(const std::shared_ptr<Factory> &f) {
