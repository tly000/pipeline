...
std::shared_ptr<RawKernel> CPUFactory::createKernel(const std::string &progName, const std::string &kernelName,
                                                    const std::string &compilerParams) {
    ...
    std::pair<int, std::string> output =
        systemCommand("g++ -O3 -shared -fPIC -std=c++20 " + compilerParams + " \"" + filePath +
                      "\""
                      " -o \"" +
                      libPath + "\"");
    if (output.first) {
    ...
    return std::make_shared<CPUKernel>(library,
                                       reinterpret_cast<CPUKernel::KernelFunc>(library.loadSymbol(kernelName)));
}
